...
#pragma pack(pop)
	...
	r1 = _mm256_set1_ps(f_id); /* all values to id */
	r3 = _mm256_set1_ps(100.0f);
	r1 = _mm256_mul_ps(r1, r3);
	
	f_pi = GetCurrentProcessorNumber();
	r2 = _mm256_set1_ps(f_pi);
	r2 = _mm256_add_ps(r2, r2);
	
	ms = rand() % 500;
	Sleep(ms);
	
	r3 = _mm256_set1_ps(2.0f);
	r2 = _mm256_div_ps(r2, r3);
	
	_mm256_store_ps(res, r1);
	data->procresult = res[7];
	_mm256_store_ps(res, r2);
	data->procindex = res[7];
...
int main(int argc, char *argv[])
{
	...
	{
		fprintf(stderr, "AVX not supported, sorry\n");
		return EXIT_FAILURE;
	}
